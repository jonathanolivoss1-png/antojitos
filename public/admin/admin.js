(function () {
  ...
  const cashClosingDateInput = document.getElementById('cashClosingDateInput');
  const cashClosingRefreshBtn = document.getElementById('cashClosingRefreshBtn');
  const cashClosingPrintBtn = document.getElementById('cashClosingPrintBtn');
  const cashClosingSaveBtn = document.getElementById('cashClosingSaveBtn');
  const cashClosingStatus = document.getElementById('cashClosingStatus');
  const cashGrossSales = document.getElementById('cashGrossSales');
  const cashValidOrders = document.getElementById('cashValidOrders');
  const cashAverageTicket = document.getElementById('cashAverageTicket');
  const cashShippingFees = document.getElementById('cashShippingFees');
  const cashDeliveredSales = document.getElementById('cashDeliveredSales');
  const cashPendingSales = document.getElementById('cashPendingSales');
  const cashCancelledOrders = document.getElementById('cashCancelledOrders');
  const cashTopProduct = document.getElementById('cashTopProduct');
  const cashNetAfterExpenses = document.getElementById('cashNetAfterExpenses');
  const cashDifference = document.getElementById('cashDifference');
  const cashExpensesInput = document.getElementById('cashExpensesInput');
  const cashCountedInput = document.getElementById('cashCountedInput');
  const cashNotesInput = document.getElementById('cashNotesInput');
...
// === HISTORIAL_DIARIO_PATCH_V1: pedidos históricos de solo lectura ===
  ...
  function getCashClosingDate() {
    const value = String(
      cashClosingDateInput?.value ||
      getSelectedDateKey()
    ).trim();
  ...
  function renderCashClosing(result) {
    currentCashClosing = result || null;
    ...
    setTextValue(
      cashGrossSales,
      formatMoney(summary.grossSales || 0)
    );

    setTextValue(
      cashValidOrders,
      String(summary.validOrders || 0)
    );

    setTextValue(
      cashAverageTicket,
      formatMoney(summary.averageTicket || 0)
    );

    setTextValue(
      cashShippingFees,
      formatMoney(summary.shippingFees || 0)
    );

    setTextValue(
      cashDeliveredSales,
      formatMoney(summary.deliveredSales || 0)
    );

    setTextValue(
      cashPendingSales,
      formatMoney(summary.pendingSales || 0)
    );

    setTextValue(
      cashCancelledOrders,
      String(summary.cancelledOrders || 0)
    );
    ...
    setTextValue(
      cashNetAfterExpenses,
      formatMoney(summary.netAfterExpenses || 0)
    );

    setTextValue(
      cashDifference,
      formatMoney(summary.cashDifference || 0)
    );

    if (cashExpensesInput) {
      cashExpensesInput.value =
        Number(closing.expenses || 0).toFixed(2);
    }

    if (cashCountedInput) {
      cashCountedInput.value =
        Number(closing.countedCash || 0).toFixed(2);
    }

    if (cashNotesInput) {
      cashNotesInput.value =
        String(closing.notes || '');
    }

    if (cashClosingStatus) {
      const savedText = closing.updatedAt
        ? ` Guardado: ${formatDate(closing.updatedAt)}.`
        : ' Este corte aún no se ha guardado.';

      cashClosingStatus.textContent =
        `Corte del ${result?.date || getCashClosingDate()}: ${summary.validOrders || 0} pedidos válidos, ${summary.deliveryOrders || 0} a domicilio y ${summary.pickupOrders || 0} de recolección.${savedText}`;
    }
  }

  async function loadCashClosingData(options = {}) {
    if (!cashClosingDateInput) return null;

    if (
      !cashClosingDateInput.value ||
      options.syncWithDashboard
    ) {
      cashClosingDateInput.value =
        getSelectedDateKey();
    }

    const date = getCashClosingDate();

    if (cashClosingRefreshBtn) {
      cashClosingRefreshBtn.disabled = true;
    }

    try {
      const result = await api(
        `/api/admin/cash-closing?date=${encodeURIComponent(date)}`
      );

      renderCashClosing(result);
      return result;
    } catch (error) {
      if (!options.silent) {
        showToast(
      ...
      if (cashClosingRefreshBtn) {
        cashClosingRefreshBtn.disabled = false;
      }
    }
  }
  ...
  async function saveCashClosing() {
    ...
    const expenses = Math.max(
      0,
      Number(cashExpensesInput?.value || 0)
    );

    const countedCash = Math.max(
      0,
      Number(cashCountedInput?.value || 0)
    );
    ...
    if (cashClosingSaveBtn) {
      cashClosingSaveBtn.disabled = true;
    }

    try {
      const result = await api(
        '/api/admin/cash-closing',
        {
          method: 'PUT',
          body: JSON.stringify({
            date,
      ...
      if (cashClosingSaveBtn) {
        cashClosingSaveBtn.disabled = false;
      }
    }
  }

  function escapeAdminPrint(value) {
    return String(value ?? '')
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
  ...
  function printCashClosing() {
    if (!currentCashClosing) {
      showToast(
        'Consulta primero el corte de caja',
        true
      );
      return;
    }

    const summary = currentCashClosing.summary || {};
    const closing = currentCashClosing.closing || {};
    ...
    const rows = [
      ['Fecha', currentCashClosing.date],
      ['Ventas válidas', formatMoney(summary.grossSales || 0)],
      ['Pedidos válidos', summary.validOrders || 0],
      ['Ticket promedio', formatMoney(summary.averageTicket || 0)],
      ['Envíos cobrados', formatMoney(summary.shippingFees || 0)],
      ['Ventas entregadas', formatMoney(summary.deliveredSales || 0)],
      ['Ventas pendientes', formatMoney(summary.pendingSales || 0)],
      ['Pedidos cancelados', summary.cancelledOrders || 0],
      ['Producto más vendido', summary.topProduct || 'Sin datos'],
      ['Gastos', formatMoney(closing.expenses || 0)],
      ['Resultado', formatMoney(summary.netAfterExpenses || 0)],
      ['Total contado', formatMoney(closing.countedCash || 0)],
      ['Diferencia de caja', formatMoney(summary.cashDifference || 0)]
    ];
    ...
    printWindow.document.write(`
      ...
      <html lang="es">
        <head>
          <meta charset="utf-8">
          <title>Corte de caja ${escapeAdminPrint(currentCashClosing.date)}</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 32px; color: #3d2c27; }
            h1 { color: #c73e1d; }
            table { width: 100%; border-collapse: collapse; margin-top: 20px; }
            td { border-bottom: 1px solid #ddd; padding: 10px; }
            td:last-child { text-align: right; font-weight: 700; }
            .notes { margin-top: 24px; padding: 14px; background: #fff8e1; }
          </style>
        </head>
        <body>
          <h1>Corte de caja — Antojitos Los Anafres</h1>
          <table>
            ${rows.map(([label, value]) => `
              <tr>
                <td>${escapeAdminPrint(label)}</td>
                <td>${escapeAdminPrint(value)}</td>
              </tr>
            `).join('')}
          </table>
          <div class="notes">
            <strong>Notas:</strong>
            ${escapeAdminPrint(closing.notes || 'Sin notas')}
          </div>
        </body>
      </html>
    `);
  ...
  async function downloadAdminBackup() {
    ...
    try {
      ...
      const filename = match?.[1] ||
        `respaldo-antojitos-${getTodayDateKey()}.json`;
  ...
  async function restoreAdminBackup() {
    ...
    if (file.size > 15 * 1024 * 1024) {
      showToast(
        'El respaldo supera el límite de 15 MB',
        true
      );
      return;
    }
    ...
    const confirmation = window.prompt(
      'Esta acción reemplazará pedidos y configuraciones. Escribe RESTAURAR para continuar.'
    );
    ...
    try {
      ...
      showToast(
        `Respaldo restaurado: ${Number(result.restoredRows || 0)} registros`
      );
  ...
  function bindAdvancedAdminFeatures() {
    ...
    cashClosingRefreshBtn?.addEventListener(
      'click',
      () => {
        void loadCashClosingData();
      }
    );

    cashClosingSaveBtn?.addEventListener(
      'click',
      () => {
        void saveCashClosing();
      }
    );

    cashClosingPrintBtn?.addEventListener(
      'click',
      printCashClosing
    );

    dayFilterInput?.addEventListener(
      'change',
      () => {
        if (cashClosingDateInput) {
          cashClosingDateInput.value =
            getSelectedDateKey();
        }
    ...
    cashClosingDateInput?.addEventListener(
      'change',
      () => {
        void loadCashClosingData();
      }
      ...
      () => {
        if (restoreBackupFileName) {
          restoreBackupFileName.textContent =
            restoreBackupInput.files?.[0]?.name ||
    ...
    restoreBackupBtn?.addEventListener(
      'click',
      () => {
        void restoreAdminBackup();
      }
    );

    if (
      cashClosingDateInput &&
      !cashClosingDateInput.value
    ) {
      cashClosingDateInput.value =
        getSelectedDateKey();
